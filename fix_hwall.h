...
/*
Changes:
-Changed all WALL to HWALL
-Changed all FixWall to FixHwall
---Working till here

-Added chi, cstyle, cindex and cstr

*/


#ifndef LMP_FIX_HWALL_H
#define LMP_FIX_HWALL_H

#include "fix.h"

namespace LAMMPS_NS {

class FixHwall : public Fix {
 public:
  int nwall;
  int wallwhich[6];
  double coord0[6];
  ...
  FixHwall(class LAMMPS *, int, char **);
  virtual ~FixHwall();
  int setmask();
  virtual void init();
  void setup(int);
 ...
 protected:
  double epsilon[6],sigma[6],chi[6],cutoff[6];		//added chi
  double ewall[7],ewall_all[7];
  double xscale,yscale,zscale;
  int estyle[6],sstyle[6],cstyle[6],wstyle[6];		//added cstyle Is chi variable or constant in the simulation
  int eindex[6],sindex[6],cindex[6];			//added cindex Index for the variable chi functionality
  char *estr[6],*sstr[6],*cstr[6];			//added cstr   String for the variable chi functionality
