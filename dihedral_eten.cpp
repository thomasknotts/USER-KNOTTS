...
DihedralEten::DihedralEten(LAMMPS *lmp) : Dihedral(lmp)
{
  weightflag = 0;
  writedata = 1;
}
...
void DihedralEten::compute(int eflag, int vflag)
{
...
void DihedralEten::coeff(int narg, char **arg)
{
  ...
  // for eten, the shift can be a double.  This is unlike other
  // charmm forcefields.  LAMMPS, by default, wants the shift 
  // to an integer for charmm dihedrals.  This is changed here.
...
void DihedralEten::init_style()
{
  ...
  if (weightflag) {
    int itmp;
    if (force->pair == NULL)
      error->all(FLERR,"Dihedral eten is incompatible with Pair style");
...
void DihedralEten::write_restart(FILE *fp)
{
...
void DihedralEten::read_restart(FILE *fp)
{
  allocate();
...
void DihedralEten::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ndihedraltypes; i++)
